...
class BasePage:
    ...
    def right_to_left_swipe(self):
        try:
            startX = self.device_width* 8 / 9
            startY = self.device_height/2
            endX = self.device_width/9
            endY = self.device_height/2
            self.action.long_press(None, startX, startY).move_to(None, endX, endY).release().perform()
        except Exception as e:
            raise e
